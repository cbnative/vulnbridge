...
That translation is all vulnscout does. Every finding, from any supported tool, comes out like this:
...
You only need Python 3.10 or newer. There is no binary to download: `pip` builds the `vulnscout` command from this repo's source, and you can try it on the bundled example reports without installing any scanner.

**1. Clone the repo and enter it**

```bash
git clone https://github.com/cbnative/vulnscout.git
cd vulnscout
```
...
**3. Install vulnscout into it**

```bash
pip install -e .
```

This reads `pyproject.toml` (the Python equivalent of a `package.json`) and creates the `vulnscout` command inside `.venv`. The `-e` makes the install editable, so if you change the source the command picks it up without reinstalling.

**4. Check it worked**

```bash
vulnscout normalize --tool grype examples/grype-alpine.json | head -14
```
...
Run any supported scanner with JSON output, then hand the file to vulnscout:

```bash
trivy image --format json --output trivy-report.json alpine:3.18
vulnscout normalize --tool trivy trivy-report.json -o findings.json
```
...
When you come back later in a new terminal, activate the environment again (`source .venv/bin/activate`) and the `vulnscout` command is back.
